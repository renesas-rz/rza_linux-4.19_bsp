...
/*-------------------------------------------------------------------------
 * Add a dave flush command into current dlist */
d2_s32 d2_insertwaitdlist_intern( d2_device *handle )
{
   D2_VALIDATE( handle, D2_INVALIDDEVICE); /* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/

   /* copy scratch buffer to dlist (dlist cache flush) */
   d2_scratch2dlist_intern( handle );
   /* insert 0x04 wait into dlist to wait for writeback complete */
...
/*-------------------------------------------------------------------------
 * Add a dave flush command into current dlist */
d2_s32 d2_insertwaitpipelinedlist_intern( d2_device *handle )
{
   D2_VALIDATE( handle, D2_INVALIDDEVICE); /* PRQA S 4130, 3112 */ /* $Misra: #DEBUG_MACRO $*/
