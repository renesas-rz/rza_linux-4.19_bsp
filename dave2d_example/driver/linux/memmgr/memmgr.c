...
#define MINIMUM_BLOCKSIZE MEMMGR_SIZE_ALIGN*64
...
void memmgr_initheapmem(memmgr *mmgr, void *base, size_t size )
{
  ...
  mmgr->ctrlblk  = (d0_heap*)base;
  /* test alignment and minimal size*/
  assert( ! (((unsigned int)base | size) & (MEMMGR_SIZE_ALIGN-1)) );
  assert( size > ( sizeof(memblk) + sizeof(d0_heap)));
  mmgr->ctrlblk->base = mmgr->ctrlblk + 1;
  mmgr->ctrlblk->end  = (char*)(mmgr->ctrlblk->base) + size - 1 ;
  ((memblk*)(mmgr->ctrlblk->base))->lastblk = NULL;
  ((memblk*)(mmgr->ctrlblk->base))->size = size - sizeof(memblk);
}
...
void * memmgr_alloc(memmgr *mmgr, size_t size)
{
  ...
  size = ( size + MEMMGR_SIZE_ALIGN ) & ~(MEMMGR_SIZE_ALIGN-1);
